...
bool VirtualFileSystem::ResolveSymbolicLink(const std::string_view path, std::string& result) {
  ...
  // Xbox 360 titles frequentemente abrem arquivos pelo nome puro (sem letra
  // de drive), esperando que resolvam contra o device padrao do titulo (D:).
  // O xboxkrnl.exe real faz isso implicitamente; emulamos aqui.
  if (!was_resolved && result.find(':') == std::string::npos) {
    auto it = std::find_if(symlinks_.cbegin(), symlinks_.cend(), [&](const auto& s) {
      return rex::string::utf8_starts_with_case(s.first, "d:");
    });
    if (it != symlinks_.cend()) {
